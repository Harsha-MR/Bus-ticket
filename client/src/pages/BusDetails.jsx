  ...
  useEffect(() => {
    const fetchBusDetails = async () => {
      try {
        ...
        console.log("from busDetails",response.data);
...
function BusDetails() {
  ...
  // Check if bus and its decks are valid before rendering
  if (!bus || !bus.lowerDeck || !bus.upperDeck) {
        ...
        <div className="flex gap-8 mb-8">
          <div className="w-3/4">
            <div className="grid grid-cols-5 gap-4">
              {(selectedDeck === 'lower' ? bus.lowerDeck : bus.upperDeck).map(
                (seat) => (
                  <button
                    key={seat.id}
